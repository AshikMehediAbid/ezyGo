// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using ezyGo.Admin.Storage.Sql;

#nullable disable

namespace ezyGo.Admin.Storage.Migrations
{
    [DbContext(typeof(AdminDbContext))]
    [Migration("20251208131546_AddIsAutoScheduled-PropertyInTripTemplateTabel")]
    partial class AddIsAutoScheduledPropertyInTripTemplateTabel
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusCompanyEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("OwnerName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("RegistrationNo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("BusCompanies");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("BusCompanyEntityId")
                        .HasColumnType("int");

                    b.Property<string>("BusName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusRegNo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("BusType")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("DriverName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("TotalCapacity")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("BusCompanyEntityId");

                    b.ToTable("Buses", (string)null);
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusStationEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("StationDescription")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("StationName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("BusStations");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.RouteEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int?>("EndingPointId")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int?>("StartingPointId")
                        .HasColumnType("int");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("EndingPointId");

                    b.HasIndex("StartingPointId");

                    b.ToTable("Routes", (string)null);
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.RouteStoppageEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("BusStationEntityId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<int>("RouteEntityId")
                        .HasColumnType("int");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("BusStationEntityId");

                    b.HasIndex("RouteEntityId");

                    b.ToTable("RouteStoppages", (string)null);
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.TrainStation", b =>
                {
                    b.Property<int>("TrainStationId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("TrainStationId"));

                    b.Property<string>("TrainStationDescription")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrainStationName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("TrainStationId");

                    b.ToTable("TrainStations");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.TripTemplate", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<TimeOnly>("ArrivalTime")
                        .HasColumnType("time");

                    b.Property<int>("BaseFare")
                        .HasColumnType("int");

                    b.Property<int?>("BusId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<TimeOnly>("DepartureTime")
                        .HasColumnType("time");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAutoScheduled")
                        .HasColumnType("bit");

                    b.Property<int?>("RouteId")
                        .HasColumnType("int");

                    b.Property<string>("Stoppages")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("BusId");

                    b.HasIndex("RouteId");

                    b.ToTable("TripTemplates", (string)null);
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusEntity", b =>
                {
                    b.HasOne("ezyGo.Admin.Storage.Entities.BusCompanyEntity", "Company")
                        .WithMany("Buses")
                        .HasForeignKey("BusCompanyEntityId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Company");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusStationEntity", b =>
                {
                    b.OwnsOne("ezyGo.Admin.Storage.Entities.Geo", "Geo", b1 =>
                        {
                            b1.Property<int>("BusStationEntityId")
                                .HasColumnType("int");

                            b1.Property<decimal>("Latitude")
                                .HasColumnType("decimal(18,2)");

                            b1.Property<decimal>("Longitude")
                                .HasColumnType("decimal(18,2)");

                            b1.HasKey("BusStationEntityId");

                            b1.ToTable("BusStations");

                            b1.WithOwner()
                                .HasForeignKey("BusStationEntityId");
                        });

                    b.Navigation("Geo");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.RouteEntity", b =>
                {
                    b.HasOne("ezyGo.Admin.Storage.Entities.BusStationEntity", "EndingPoint")
                        .WithMany()
                        .HasForeignKey("EndingPointId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("ezyGo.Admin.Storage.Entities.BusStationEntity", "StartingPoint")
                        .WithMany()
                        .HasForeignKey("StartingPointId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("EndingPoint");

                    b.Navigation("StartingPoint");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.RouteStoppageEntity", b =>
                {
                    b.HasOne("ezyGo.Admin.Storage.Entities.BusStationEntity", "BusStationEntity")
                        .WithMany("RouteStoppages")
                        .HasForeignKey("BusStationEntityId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ezyGo.Admin.Storage.Entities.RouteEntity", "RouteEntity")
                        .WithMany("Stoppages")
                        .HasForeignKey("RouteEntityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BusStationEntity");

                    b.Navigation("RouteEntity");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.TrainStation", b =>
                {
                    b.OwnsOne("ezyGo.Admin.Storage.Entities.Geo", "Geo", b1 =>
                        {
                            b1.Property<int>("TrainStationId")
                                .HasColumnType("int");

                            b1.Property<decimal>("Latitude")
                                .HasColumnType("decimal(18,2)");

                            b1.Property<decimal>("Longitude")
                                .HasColumnType("decimal(18,2)");

                            b1.HasKey("TrainStationId");

                            b1.ToTable("TrainStations");

                            b1.WithOwner()
                                .HasForeignKey("TrainStationId");
                        });

                    b.Navigation("Geo")
                        .IsRequired();
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.TripTemplate", b =>
                {
                    b.HasOne("ezyGo.Admin.Storage.Entities.BusEntity", "BusEntity")
                        .WithMany()
                        .HasForeignKey("BusId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("ezyGo.Admin.Storage.Entities.RouteEntity", "RouteEntity")
                        .WithMany()
                        .HasForeignKey("RouteId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("BusEntity");

                    b.Navigation("RouteEntity");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusCompanyEntity", b =>
                {
                    b.Navigation("Buses");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.BusStationEntity", b =>
                {
                    b.Navigation("RouteStoppages");
                });

            modelBuilder.Entity("ezyGo.Admin.Storage.Entities.RouteEntity", b =>
                {
                    b.Navigation("Stoppages");
                });
#pragma warning restore 612, 618
        }
    }
}
